...
We denote GeoBFT with k clusters by Geo-k. It is clear that as the system scales, the throughput, and latency decrease. 

With respect to throughput, the performance of Geo-2 is very close to PBFT, just slightly better. While Geo-4 outperforms Geo-2 and PBFT at all replica settings. The reason Geo-2 has almost the same performance as PBFT is that there is extra communication cost among clusters, and when there are only two clusters in the system, the effect of this extra cost is more significant. When the number of clusters increases to 4, we can notice that the benefit of the GeoBFT protocol has outweighed the cost.

In general, the latency of Geo-2 and Geo-4 is smaller than PBFT. Only the latency of Geo-4 is slightly larger than PBFT at the 16-replica setting, this is also because of the extra cost of inter-cluster sharing under the GeoBFT protocol. As the number of replicas increases, the latency of Geo-2 and Geo-4 becomes greatly smaller than PBFT latency, the latency of Geo-4 is almost half of PBFT latency at the 128-replica setting.
...
In the second experiment, we measure the performance of GeoBFT when a single primary fails (in one of the two regions). Figure 9 shows the throughput attained by GeoBFT when the primary of one cluster fails, and the replicas run the remote view change protocol to replace the faulty primary. The primary of one of the clusters fails at 30s, and the system’s average throughput starts decreasing, then drops down to 0 at 45s; Later, GeoBFT again observes an increase in throughput at 110s.
