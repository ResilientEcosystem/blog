...
In the second experiment, we measure the performance of GeoBFT when a single primary fails (in one of the two regions). Figure 9 shows the throughput attained by GeoBFT when the primary of one cluster fails, and the replicas run the remote view change protocol to replace the faulty primary. The primary of one of the clusters fails at 30s, and the system’s average throughput starts decreasing; Later, GeoBFT again observes an increase in throughput at 50s. GeoBFT with remote view change protocol takes more memory to run compared to normal-case GeoBFT, hence, it causes a decrease in throughput when running under the same experiment setup.
