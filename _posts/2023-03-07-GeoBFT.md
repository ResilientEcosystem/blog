...
Once a cluster has completed local replication of a client request, it proceeds with the second step: sharing the client request with all other clusters. After $C$ reaches local consensus on client request $\langle T\rangle_{P_{C}}$ in round $\rho$—enabling construction of the commit certificate $[\langle T \rangle_{P_{C}}, \rho]$ that proves local consensus— $C$ needs to exchange this client request and the accompanying proof with all other clusters. The following graphs shows the global sharing protocol used by $C_{1}$ to send $m := (\langle T \rangle_{P_{c}}, [ \langle T \rangle_{P_{c}}, \rho ])$ to $C_{2}$ and the pseudo-code of this protocol, which we will illustrate next.
...
The first experiment is designed to test the maximum throughput and latency of GeoBFT and compare the result with PBFT performance under the same configuration. The throughput and latency of PBFT and GeoBFT with 2 clusters and 4 clusters with different numbers of replicas are recorded. The replica number is set to 16, 32, 64 and 128.

We denote GeoBFT with k clusters by Geo-k. It is clear that as the system scales, the throughput and latency decreases. 

With respect to throughput, the performance of Geo-2 is very close to PBFT, just slightly better. While Geo-4 outperforms Geo-2 and PBFT at all replica settings. The reason Geo-2 has almost the same performance as PBFT is because there is extra communication cost among clusters, and when there are only two clusters in the system, the effect of this extra cost is more significant. When the number of clusters increase to 4, we can notice that the benefit of GeoBFT protocol has outweighed the cost.

In general, the latency of Geo-2 and Geo-4 is smaller than PBFT. Only the latency of Geo-4 is slightly larger than PBFT at 16-replica setting, this is also because the extra cost of inter-cluster sharing under GeoBFT protocol. As the number of replica increases, the latency of Geo-2 and Geo-4 becomes greatly smaller than PBFT latency, the latency of Geo-4 is almost half of PBFT latency at 128-replica setting.

<p>
    <img src="{{ site.baseurl }}/assets/images/GeoBFT/Performance Tpt.png" alt="Cover photo" style="width: 80%"/>
    <img src="{{ site.baseurl }}/assets/images/GeoBFT/Performance Latency.png" alt="Cover photo" style="width: 80%"/>
    <br>
