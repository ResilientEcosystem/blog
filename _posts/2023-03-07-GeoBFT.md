...
Let $m$ = ($\langle T\rangle_{P_{C}}$, [$\langle T\rangle_{P_{C}}$, $\rho$]$_{C_{1}}$) be the message that some replica in cluster $\mathcal{C_{1}}$ needs to send to some replicas $C_{2}$.
