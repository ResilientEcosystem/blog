...
1. **Create Transactions**:
   ResCash allows users to create detailed financial transactions with fields such as amount, category, transaction type, merchant, and payment method. Transactions are stored securely in ResilientDB for data integrity and MongoDB for efficient indexing. The platform also integrates a real-time currency exchange API, enabling users to input amounts in multiple currencies.
...
5. **Budgeting and Reporting Tools**:
   The platform empowers users to define budgets across transaction categories, track real-time spending, and compare expenses against set limits. Reporting features allow users to generate summaries for customizable date ranges, providing insights into financial trends.
...
- **Alex Chenn** [[ymfeng@ucdavis.edu]](yscchen@ucdavis.edu)
