...
- In the the **Pre-Prepare phase** we see the function `WorkerThread::process_client_batch` processing of a client request. The primary will propose transactions through broadcasting its `BatchRequests` message, an aggregation of transactions, to all the other replicas representing initiation of the consensus protocol.
